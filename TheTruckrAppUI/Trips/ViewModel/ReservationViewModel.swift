...
class ReservationViewModel: NSObject, ObservableObject {
    ...
    @Published var numberOfTrucks = 1
    @Published var kindOfTruck: TruckKind = .triaxle
    ...
    //MARK: Fetch Charge For App
    func calculateCharge() -> Double {
        if UserService.shared.currentUser?.accountType == .dispatcher {
            let fee = 12
            return tripService.calculateCharge(numberOfTrucks: numberOfTrucks, fee : fee)
        } else {
            let fee = 10
            return tripService.calculateCharge(numberOfTrucks: numberOfTrucks, fee : fee)
        }
...
extension ReservationViewModel: MKLocalSearchCompleterDelegate {
